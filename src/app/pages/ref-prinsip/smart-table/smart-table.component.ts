...
export class SmartTableComponent {
  ...
  onCreateConfirm(event): void {
    ...
    this.prefix = event.newData.prefix;
    this.prinsip = event.newData.prinsip;
    if (this.id == ""){
      this.showToast("warning", "Kolom ID masih Kosong", "Harus di isi");
    }
    else if (this.prefix == ""){
      this.showToast("warning", "Kolom Prefix masih Kosong", "Harus di isi");
    }
    else if (this.prinsip == ""){
      this.showToast("warning", "Kolom Prinsip masih Kosong", "Harus di isi");
    }
    else{
    this.httpClient.post(this._global.baseAPIUrl + '/Itk_ref_prinsips',event.newData).subscribe(data  => {
  ...
  onSaveConfirm(event): void {
    ...
    this.httpClient.put(this._global.baseAPIUrl + '/Itk_ref_prinsips/'+event.data.id,event.newData).subscribe(data  => {
  ...
  onDeleteConfirm(event): void {
    if (window.confirm('Are you sure you want to delete?')) {
      this.httpClient.delete(this._global.baseAPIUrl + '/Itk_ref_prinsips/'+event.data.id).subscribe(data => {
        event.confirm.resolve();
